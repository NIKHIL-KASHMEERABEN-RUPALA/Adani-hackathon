...
export default function SignUp() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const success = await signup(name, email, password); // Assuming signup now takes password
            if (success) {
                toast.success("Account created successfully!");
                navigate("/");
            } else {
                toast.error("Email already in use or invalid credentials"); // More generic error
            }
        } catch (error) {
            ...
            <Card className="w-full max-w-md border-4 border-black shadow-[8px_8px_0px_0px_#000] rounded-none">
                <CardHeader className="space-y-2 text-center pb-8 border-b-4 border-black">
                    ...
                    <CardDescription className="text-muted-foreground font-mono uppercase text-xs">
                        Join the network to start monitoring assets
                    </CardDescription>
                </CardHeader>
                <CardContent className="pt-8 space-y-6">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="space-y-2">
                            <Label htmlFor="name" className="font-mono uppercase font-bold text-xs">Full Name</Label>
                            <Input
                                id="name"
                    ...
                    <div className="mt-4 text-center text-sm font-mono">
                        ...
                        <Link to="/login" className="underline underline-offset-4 hover:text-primary font-bold decoration-2">
                            Login
                        </Link>
                    </div>
                </CardContent>
